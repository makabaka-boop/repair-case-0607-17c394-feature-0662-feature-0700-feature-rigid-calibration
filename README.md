...
- 前端对每次“新提交 / 字段校验失败 / 请求失败 / 重置”立即作废旧结论（含旧区间与标定摘要），
  并用请求序号忽略在途旧响应——**乱序返回的旧响应不能恢复旧区间**。

## 全站仪坐标系标定（可选）

复测时钻孔中心可能来自全站仪坐标系，而电缆路径仍使用施工局部坐标。请求可携带
`calibration`：`survey_points` 与 `path_points` 按顺序给出 **2～20 对**控制点，
并设置正数 `max_rms_error`。两组坐标必须有限且等长，且各自不能全部重合；后端在
未舍入双精度下求 survey → path 的二维最小二乘**保距刚体变换**：

- 只允许旋转和平移，不估计尺度、禁止镜像；非共线镜像数据返回明确定位的 422；
- 成功响应返回 `rotation`（2×2 行优先矩阵）、`translation`、`rms_error`
  及逐点 `point_residuals`，这些标定值保持未舍入双精度；
- 标定成功后**只变换禁入圈圆心**，禁入圈半径、电缆路径节点与电缆半径均不变；
- 随后复用现有空间粗筛、精确闭集求交、连续区间与复合侵入扫描链路；碰撞、
  区间拓扑、复合段、SVG 高亮和三位展示全部来自同一批未舍入几何结果；
- 退化控制点、镜像对应或 RMS 超过 `max_rms_error` 时返回字段定位的 422，
  不生成任何碰撞、侵入或复合侵入结果；
- 省略 `calibration` 时，请求与响应均逐项兼容（响应中也不出现 `calibration` 字段）。
...
- 测试：pytest（穿越/端点/相切/圈内线段/排序/字段错误/独立矩阵核对标定/连续侵入区间合并与独立边界/
  粗筛不漏候选/20000×2000 稀疏性能）、Vitest + Testing Library
  （录入校验、**真实 HTTP 请求**、首个碰撞高亮、标定摘要/残差、侵入区间明细与 SVG 片段高亮、
...
api/                       FastAPI 服务
  ...
  app/calibration.py       自实现二维：最小二乘 proper rotation + translation（双精度、大坐标稳定）
  app/schemas.py           Pydantic 模型与字段级校验
  app/main.py              /api/precheck、/api/health、422 字段错误
  tests/                   pytest（标定、碰撞、区间语义、合并边界、性能与错误）
web/                       React + Vite
...
- 路径节点与禁入圈圆心输入为整数毫米；电缆/禁入圈半径为正数。
- 标定控制点可以使用有限小数毫米；`circles[].x/y` 是全站仪坐标，成功后仅圆心变换。
- 省略可选 `calibration` 时，旧请求与旧响应逐项兼容。
...
含标定时，`circles`、碰撞、区间与 SVG 均以**变换后的禁入圈圆心**计算，路径仍为原始
施工局部坐标。标定时额外返回：
...
```json
{
  "calibration": {
    ...
    "translation": {"x": -990.0, "y": -1980.0},
    "rms_error": 0.0,
    "max_rms_error": 0.01,
    "point_residuals": [
      {
        "index": 0,
        "survey_point": {"x": 1000.0, "y": 2000.0},
        "path_point": {"x": 10.0, "y": 20.0},
        "residual": 0.0
      }
    ]
  }
...
非整数毫米坐标、布尔值、多余字段、标定控制点数量不在 2～20、两组不等长、任一组全部
重合、非共线镜像或 RMS 超阈值等，均返回字段级错误且**不产生任何预检结论**；
前端会清除旧结论并标红对应字段：
